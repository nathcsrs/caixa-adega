...
async function syncEntriesToCloud() {
  ...
  try {
    const rows = state.entries.map(localEntryToCloud).filter((row) => row.amount_cents > 0);
    if (rows.length) {
...
async function loadCloudData() {
  ...
  try {
    ...
    const cloudEntries = (entriesResult.data || []).map(cloudEntryToLocal);
    const cloudMeta = Object.fromEntries(
      (metaResult.data || []).map((row) => [row.date, cloudMetaToLocal(row)]),
    );

    state.entries = sameOrFirstAccount ? mergeEntries(state.entries, cloudEntries) : normalizeEntries(cloudEntries);
...
async function drawClosingCanvas() {
  ...
  const logo = await loadImage("./public/logo-adega.png");
  if (logo) {
    const logoWidth = 185;
  ...
  ctx.fillStyle = "#1a50d8";
  roundRect(ctx, 782, 442, 132, 138, 18);
  ctx.fill();
  ctx.lineWidth = 9;
  ctx.strokeStyle = "#05050a";
  ctx.stroke();
  ctx.fillStyle = "#5fdcff";
  roundRect(ctx, 808, 468, 75, 82, 14);
  ctx.fill();
  ctx.strokeStyle = "#05050a";
  ctx.lineWidth = 6;
  ctx.stroke();
  ctx.fillStyle = "#ffd447";
  ctx.beginPath();
  ctx.arc(846, 510, 26, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "#05050a";
  ctx.lineWidth = 6;
  ctx.stroke();
  ...
  ctx.shadowOffsetX = 7;
  ctx.shadowOffsetY = 7;
  drawText(ctx, formatCurrency(stats.totals.general), 118, 580, {
    color: "#05050a",
    size: 106,
    weight: 900,
  });
  ...
  ctx.shadowOffsetX = 0;
  ctx.shadowOffsetY = 0;
  drawText(ctx, `${stats.entries.length} ${stats.entries.length === 1 ? "entrada registrada" : "entradas registradas"}`, 118, 662, {
    color: "#5a5a78",
    size: 28,
    weight: 800,
  });
  ...
  shiftCards.forEach((card, index) => {
    ...
    ctx.strokeStyle = "#05050a";
    ctx.stroke();
    drawText(ctx, card[0], x + 34, y + 68, {
      color: "#05050a",
      size: 29,
      weight: 900,
    });
    drawText(ctx, formatCurrency(card[1]), x + 34, y + 125, {
      color: "#05050a",
      size: 36,
      weight: 900,
    });
